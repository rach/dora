...
> macOS Apple Silicon (M1 / M2 / M3+). Intel Mac and Linux: build from source via `cargo install --git https://github.com/rach/dora --tag v0.10.0`.
...
```sh
curl -L -o /tmp/dora https://github.com/rach/dora/releases/latest/download/dora-fs-v0.10.0-macos-arm64
chmod +x /tmp/dora
...
dora migrate                                                     # move pre-0.9 co-located .dora/ indexes into ~/.dora
dora calibrate [<name>]                                          # derive a per-source confidence floor from the index
